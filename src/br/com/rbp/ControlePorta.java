...
public class ControlePorta {
	...
	public int receberDados(int id) {
		int retorno = 0;
		try {
			retorno = serialIn.read();
		} catch (IOException ex) {
			System.out.println("Não foi possível receber o dado. ");
		}
		return retorno;
	}

	public boolean retornoSucesso() {
		int retorno = 1;
		try {
			retorno = serialIn.read();
		} catch (IOException ex) {
			JOptionPane.showMessageDialog(null, "Não foi possível receber o dado. ", "Receber dados",
					JOptionPane.PLAIN_MESSAGE);
		}
		return retorno == 1;
	}

	public byte[] lerDigital(int id) throws IOException {
		byte[] b = IOUtils.toByteArray(serialIn);
		serialIn.close();
		return b;

	}

	public void escreverImagem(byte[] bytesOriginal) {
		try {
			serialOut.write(bytesOriginal);// escreve o valor na porta serial
		} catch (IOException ex) {
			System.out.println("Não foi possível enviar o dado. ");
		}
	}
